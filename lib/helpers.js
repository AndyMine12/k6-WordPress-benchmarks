...
export function generateUsername(usernameBase, start, end) {
    ...
    //! Modify username generation algorithm
...
export function checkHttpsProtocol(url){
    if (url.startsWith('https://')) {
        return url
            ...
            //force https
            //! Do not force https, since local nginx server is listening on port 80
...
export function addToCart(url,sku,id,quantity, data, metrics) {
    //TEST url lookup
        console.log("Path to all evil --> " + url)
    const defaultHeaders = {
                "content-type":
                "application/x-www-form-urlencoded;type=content-type;mimeType=application/x-www-form-urlencoded",
            };
    ...
    //TEST i suffer lots and lots (json lookup)
        console.log("Source of all evil")
